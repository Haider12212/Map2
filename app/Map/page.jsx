...
import VideoAfg from "../components/AfghanistanVideo";
import "./mapStyles.css";

const Page = () => {
  const [isSidebarOpen, setIsSidebarOpen] = useState(false);
  const [zoomPan, setZoomPan] = useState({ zoom: 1, panX: 0, panY: 0 });
  const [showImage, setShowImage] = useState(true);
  const screenSaverTimeout = 5000; // 5 seconds
  const [interacted, setInteracted] = useState(false);
  const [showScreenSaver, setShowScreenSaver] = useState(true); // Show the screen saver initially
  ...
  useEffect(() => {
    ...
    const resetTimer = () => {
      ...
      clearTimeout(timer);
      timer = setTimeout(() => setShowScreenSaver(true), screenSaverTimeout);
    };
    ...
    // Attach click event to the body
    document.body.addEventListener("click", handleClick);

    // Set initial timer to show the screen saver
    timer = setTimeout(() => setShowScreenSaver(true), screenSaverTimeout);
  ...
  useEffect(() => {
    // This effect runs every time 'showScreenSaver' or 'interacted' changes
    if (!interacted) {
      // If not interacted, show the screen saver again after 5 seconds
      const timer = setTimeout(() => {
        setShowScreenSaver(true);
      }, screenSaverTimeout);

      // Clean up the timer on unmount or when 'interacted' becomes true
      return () => clearTimeout(timer);
    }
  }, [interacted, showScreenSaver]);
  ...
  const handleCountryClick = (event) => {
    // Get the clicked country's ID
    const countryID = event.target.getAttribute("id");
  
    // Calculate the new zoom and pan values based on the clicked country
    // For simplicity, you can customize these values as needed
    const newZoom = 10;
    const newPanX = -300;
    const newPanY = 150;
    ...
    if (isSidebarOpen) {
      // Sidebar is open, set the zoom and pan back to initial values
      setZoomPan({ zoom: 1, panX: 0, panY: 0 });
  
      // Show the image again
      handleShowImage();
    } else {
      // Sidebar is closed, apply the new zoom and pan values
  ...
  return (
    <div>
    {playVideo ? (
      <VideoAfg setPlayVideo={setPlayVideo} />
    ) : (
    <>
    <div className="relative">
      <Sidebar isOpen={isSidebarOpen} onClose={toggleSidebar} toggleVideo={toggleVideo} />
      {showScreenSaver && !interacted && <ScreenSaver />}
      <div className="p-8 absolute">
        {showImage && (
          <div className="absolute ml-96 mb-96 z-[9999]">
            <Image
              src='./assets/map/Afghanistan.gif'
              alt="World Map"
              height={200}
              width={200}
              className="mb-96 -translate-y-[150%] translate-x-[200%] ml-96 mt-96"
